...
void HW_2DConv_Mmap_3(int8_t *pixel_in, int8_t *pixel_in2, int8_t *pixel_out,int8_t *pixel_out2, int32_t addr_reserved){
#pragma HLS INTERFACE m_axi depth = 482*272+3*3 port = pixel_in offset = slave bundle = user_axi_in register
#pragma HLS INTERFACE m_axi depth = 480*270 port = pixel_out offset = slave bundle = user_axi_out register
#pragma HLS INTERFACE m_axi depth = 482*272+3*3 port = pixel_in2 offset = slave bundle = user_axi_in2 register
#pragma HLS INTERFACE m_axi depth = 480*270 port = pixel_out2 offset = slave bundle = user_axi_out2 register

#pragma HLS INTERFACE s_axilite port = pixel_in bundle = user_axi4lite_config  register
#pragma HLS INTERFACE s_axilite port = pixel_out bundle = user_axi4lite_config  register
#pragma HLS INTERFACE s_axilite port = pixel_in2 bundle = user_axi4lite_config  register
#pragma HLS INTERFACE s_axilite port = pixel_out2 bundle = user_axi4lite_config  register
#pragma HLS INTERFACE s_axilite port = addr_reserved offset = 0xFFF0 bundle = user_axi4lite_config  register
#pragma HLS INTERFACE s_axilite port = return bundle = user_axi4lite_config  register
	...
	int8_t *in_base2 = pixel_in2;
	int8_t *filter_base = in_base + ROWS_I*COLS_I;
	int8_t *filter_base2 = in_base2 + ROWS_I*COLS_I;
	int8_t *out_base = pixel_out;
	int8_t *out_base2 = pixel_out2;

    int8_t filter_buffer[FILTER_SIZE][FILTER_SIZE];
#pragma HLS ARRAY_PARTITION variable=filter_buffer complete dim=0
    int8_t filter_buffer2[FILTER_SIZE][FILTER_SIZE];
#pragma HLS ARRAY_PARTITION variable=filter_buffer2 complete dim=0

    memcpy(&filter_buffer[0][0], filter_base, FILTER_SIZE*FILTER_SIZE*sizeof(int8_t));
    memcpy(&filter_buffer2[0][0], filter_base2, FILTER_SIZE*FILTER_SIZE*sizeof(int8_t));

    ap_linebuffer<int8_t, FILTER_SIZE, COLS_I> line_buffer;
    ap_linebuffer<int8_t, FILTER_SIZE, COLS_I> line_buffer2;

	ROW_LOOP: for(r=0; r<ROWS_I; r++){
		ap_window<int8_t,FILTER_SIZE,FILTER_SIZE> window_buffer;
		ap_window<int8_t,FILTER_SIZE,FILTER_SIZE> window_buffer2;
...
#pragma HLS PIPELINE II=1
            ...
            int8_t store_data = 0;
            int8_t store_data2 = 0;
			int8_t load_data = *(in_base+r*COLS_I+c);
			int8_t load_data2 = *(in_base2+r*COLS_I+c);

			line_buffer.shift_up(c);
			line_buffer.insert_bottom(load_data, c);
			line_buffer2.shift_up(c);
			line_buffer2.insert_bottom(load_data2, c);
			...
			if(r>=2)
			{
				...
				window_buffer2.shift_right();
				window_buffer2.insert(line_buffer2.getval(2,c), 0, 2);
				window_buffer2.insert(line_buffer2.getval(1,c), 1, 2);
				window_buffer2.insert(line_buffer2.getval(0,c), 2, 2);
			}
			...
			if(r>=2 && c>=2)
			{
				for(i=0; i<FILTER_SIZE; i++){
					for(j=0; j<FILTER_SIZE; j++){
						int8_t res = 0;
						int8_t val = window_buffer.getval(i,j)+window_buffer2.getval(i,j);
						if(filter_buffer[i][j]==0){
							res = 0;
						}else if(filter_buffer[i][j]==1){
							res = val;
						...
						int8_t res2 = 0;
						int8_t val2 = window_buffer.getval(i,j)+window_buffer2.getval(i,j);
						if(filter_buffer2[i][j]==0){
							res2 = 0;
						}else if(filter_buffer2[i][j]==1){
							res2 = val2;
						}else if(filter_buffer2[i][j]==-1){
							res2 = -val2;
						}
				...
				*(out_base2+(r-2)*COLS_O+(c-2)) = store_data2;
			}
